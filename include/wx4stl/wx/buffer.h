...
/**
 * Replace wxVector by QVector.
 */
template<typename TYPE> class wxVector : public std::vector<TYPE>
{
public:
    size_t          GetCount() const                { return std::vector<TYPE>::size(); }
    const TYPE &    Item(int _iItemIndex) const     { return std::vector<TYPE>::at(_iItemIndex); }
    void            Add(const TYPE &_rItem)         { std::vector<TYPE>::push_back(_rItem); }
    void            Empty()                         { std::vector<TYPE>::clear(); }
    size_t          Count() const                   { return std::vector<TYPE>::size(); }
};

#define wxArrayInt                              wxVector<int>
