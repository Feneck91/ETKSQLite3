...
wxRegEx::wxRegEx(const std::string & pattern, int flags)
{
    Compile(pattern, flags);
}

// Compile l'expression régulière
bool wxRegEx::Compile(const std::string & pattern, int flags)
{
...
bool wxRegEx::Matches(const std::string & text) const
{
    if (!m_isValid)
    {
        return false;
